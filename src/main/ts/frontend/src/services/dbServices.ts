...
async function fetchQueues() {
    return await fetch('http://localhost:8080/jmsRestClient/api/queues')
    .then((res: Response) => res.json())
    .then((data: Queue[]) => data);
}

async function fetchMessages(queue: Queue): Promise<Message[]> {
    const response = await fetch(`http://localhost:8080/jmsRestClient/api/queues/${queue?.name}/messages`);
...
async function deleteMessage(queue: Queue, message: Message) {
    return await fetch(`http://localhost:8080/jmsRestClient/api/queues/${queue?.name}/messages/${message.messageID}`,
    {
        method: "DELETE"
    })
