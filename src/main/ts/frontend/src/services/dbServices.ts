...
async function fetchQueues() {
    return await fetch('/jmsRestClient/api/queues')
...
async function fetchMessages(queue: Queue): Promise<Message[]> {
    const response = await fetch(`/jmsRestClient/api/queues/${queue?.name}/messages`);
...
async function deleteMessage(queue: Queue, message: Message) {
    return await fetch(`/jmsRestClient/api/queues/${queue?.name}/messages/${message.messageID}`,
    {
        method: "DELETE"
    })
